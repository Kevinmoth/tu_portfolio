...
// Obtener ID de categoría "Bots"
$categoria_query = $conn->prepare("SELECT id FROM categorias WHERE slug = 'bots'");
...
<div class="container my-5">
    <div class="text-center mb-5">
        <h1 class="display-4">
            <i class="bi bi-robot" style="color: var(--primary-blue);"></i> Bots
        </h1>
        <p class="lead text-muted">Proyectos de automatización y bots inteligentes</p>
    </div>
    
    <!-- Barra de búsqueda -->
    <div class="search-box">
        <form method="GET" action="bots.php">
    ...
    <?php if ($busqueda): ?>
        <div class="text-center mb-4">
            <p class="text-muted">
                ...
                (<?php echo $total_proyectos; ?> bot<?php echo $total_proyectos != 1 ? 's' : ''; ?>)
                <a href="bots.php" class="btn btn-sm btn-outline-secondary ms-2">Limpiar</a>
            </p>
        </div>
    <?php endif; ?>
    ...
    <?php else: ?>
        <div class="text-center py-5">
            <i class="bi bi-robot" style="font-size: 4rem; color: #ccc;"></i>
            <h3 class="mt-3">No se encontraron bots</h3>
            <p class="text-muted">
                <?php echo $busqueda ? 'Intenta con otros términos de búsqueda' : 'Aún no hay bots publicados'; ?>
            </p>
        </div>
    <?php endif; ?>
</div>
